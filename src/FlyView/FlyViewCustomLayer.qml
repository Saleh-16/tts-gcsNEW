...
Item {
    ...
    function _fmtCoord(ctl, coord) {
        ...
        if (root._coordFmt === 1) {
            if (!ctl || !ctl.zone) return "--"
            var z = ctl.zone.rawValue
            if (z < 1 || z > 60) return "--"
            return z + (ctl.hemisphere.rawValue ? "S" : "N") + "  " +
                   ctl.easting.rawValue.toFixed(0) + "E  " +
                   ctl.northing.rawValue.toFixed(0) + "N"
        }
        if (root._coordFmt === 2) {
            var s = (ctl && ctl.mgrs) ? ctl.mgrs.valueString : ""
            return (s && s.length > 0) ? s : "--"
        }
        return coord.latitude.toFixed(6) + " N  " + coord.longitude.toFixed(6) + " E"
    }
                                    ...
                                    Row {
                                        ...
                                        Text {
                                            ...
                                            TransformPositionController {
                                                id: rowCtl
                                                coordinate: (rowRoot._mi && rowRoot._mi.coordinate && rowRoot._mi.coordinate.isValid)
                                                            ? rowRoot._mi.coordinate : QtPositioning.coordinate()
