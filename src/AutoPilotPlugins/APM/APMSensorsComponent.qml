                ...
                QGCPopupDialog {
                    ...
                    onAccepted: {
                        if (_orientationDialogCalType == _calTypeAccel) {
                            ...
                            if (!northCalibrationCheckBox.checked) {
                                ...
                                // TTS: lat/lon محذوفة كحقول إدخال — حسب مواصفة MAVLink
                                // الرسمية لـ MAV_CMD_FIXED_MAG_CAL_YAW: "If Latitude and
                                // longitude are both zero then use the current vehicle
                                // location." يعني صفر = تلقائي بدون أي إدخال يدوي.
                                // خياري GCS/Map يبقون كتجاوز اختياري بس لو الطائرة نفسها
                                // ما عندها GPS صالح (northCalibrationManualPosition.visible).
                                let lat = 0
                                let lon = 0
                                ...
                                // TTS: yaw صار قابل للتعديل يدوياً بدل 0 (شمال) ثابت —
                                // نفس مرونة "MagCal Yaw" بـ Mission Planner
                                let yaw = parseFloat(northCalHeading.text)
                                if (isNaN(yaw)) yaw = 0
                    ...
                    Column {
                        ...
                        Column {
                            ...
                            // TTS: حقلي Lat/Lon محذوفين بطلب صريح — حسب مواصفة MAVLink
                            // الرسمية لـ MAV_CMD_FIXED_MAG_CAL_YAW، صفر = تلقائي (يستخدم
                            // موقع الطائرة الحالي من GPS)، فما فيه داعي إدخال يدوي.
                            // خياري "Use GCS/Map position" فوق يبقون كتجاوز اختياري بس.
                            // ── TTS: حقل Heading يدوي ────────────────────────
                            // يسمح بمعايرة الطائرة بأي اتجاه معروف (مو شمال بس)،
                            // نفس فكرة "MagCal Yaw" بـ Mission Planner. يمرَّر
                            // مباشرة كمعامل yaw لـ controller.calibrateCompassNorth
                            // (يتطلب تعديل C++ مطابق بـ APMSensorsComponentController.h/.cc
                            // لإضافة هذا المعامل للدالة، مو موجود افتراضياً بـ QGC).
                            QGCLabel {
                                width:      parent.width
                                visible:    northCalibrationCheckBox.checked
                                wrapMode:   Text.WordWrap
                                text:       qsTr("Heading (degrees, True North — vehicle's actual current heading)")
                            }
                            FactTextField {
                                id:         northCalHeading
                                visible:    northCalibrationCheckBox.checked
